<?php
	...
	$objPHPExcel->getActiveSheet()->getStyle("A2:I2")->getFill()->getStartColor()->setARGB("FF1E90FF");
	...
	$objPayable = $objRichText->createTextRun("LISTA DE SOLICITUD DE CITAS POR PROVINCIA");
	$objPayable->getFont()->setBold(true);
	//$objPayable->getFont()->setItalic(true);
	$objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_RED));
	$objPHPExcel->getActiveSheet()->setCellValue("E1", $objRichText);
	...
	//distrito
	$objRichText = new PHPExcel_RichText();
	$objPayable = $objRichText->createTextRun("DISTRITO");
	$objPayable->getFont()->setBold(true);
	$objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_WHITE) );
	...
	//provincia
	$objRichText = new PHPExcel_RichText();
	$objPayable = $objRichText->createTextRun("PROVINCIA");
	$objPayable->getFont()->setBold(true);
	$objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_WHITE) );
	...
	//departamento
	$objRichText = new PHPExcel_RichText();
	$objPayable = $objRichText->createTextRun("DEPARTAMENTO");
	$objPayable->getFont()->setBold(true);
	$objPayable->getFont()->setColor( new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_WHITE) );
    ...
    if($arrayinfo) 
       {
	        foreach ($arrayinfo as $datainfo):
	        	$objPHPExcel->getActiveSheet()->setCellValue("A".$nro, $nro-2)
		                              ...
		                              ->setCellValue("E".$nro,$datainfo["fechaactu"])
		                              ->setCellValue("F".$nro,$datainfo["servi"])
		                              ->setCellValue("G".$nro,$datainfo["descdistri"])
		                              ->setCellValue("H".$nro,$datainfo["descprov"])
		                              ->setCellValue("I".$nro,$datainfo["descdepa"]);
	...
	header('Content-Disposition: attachment;filename="ListaProvSoliCitas.xls"');
